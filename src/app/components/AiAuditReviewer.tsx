...
function AiStatusIcon({
  ...
  result: RowAuditResult | undefined;
  loading: boolean;
}) {
  if (loading)
    return <Loader2 className="w-4 h-4 animate-spin text-[#111827]" />;
...
function Gate1Pill({ status }: { status: string }) {
  if (status === "valid")
    return (
      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-success-light text-success font-archivo">
        valid
      </span>
    );
  if (status === "caution")
    return (
      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-warning-light text-warning font-archivo">
        caution
      </span>
    );
  return (
    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-destructive-light text-destructive font-archivo">
      rejected
    </span>
  );
}
...
export function AiAuditReviewer({
  results,
  onProceed,
  onSkip,
}: AiAuditReviewerProps) {
  const [auditResults, setAuditResults] = useState<Map<string, RowAuditResult>>(
    new Map(),
  );
  const [loadingRows, setLoadingRows] = useState<Set<string>>(new Set());
  const [editedStems, setEditedStems] = useState<Map<string, string>>(
    new Map(),
  );
  const [selectedKey, setSelectedKey] = useState<string | null>(
    results[0]?.rowKey ?? null,
  );
  ...
  function markLoading(rowKey: string, on: boolean) {
    setLoadingRows((prev) => {
      ...
      on ? s.add(rowKey) : s.delete(rowKey);
      return s;
    });
  }
  ...
  async function runAudit(result: ValidationResult, stemOverride?: string) {
    ...
    try {
      const auditResult = await auditRow(
        rowKey,
        canonicalItem,
        stemOverride ?? editedStems.get(rowKey),
      );
  ...
  const certified = Array.from(auditResults.values()).filter(
    (r) => r.status === "ai_certified",
  ).length;
  const rejected = Array.from(auditResults.values()).filter(
    (r) => r.status === "ai_rejected",
  ).length;
  ...
  const modalStem = modalKey
    ? (editedStems.get(modalKey) ?? modalResult?.canonicalItem?.stem ?? "")
    : "";
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      const currentIndex = selectedKey
        ? results.findIndex((r) => r.rowKey === selectedKey)
        : -1;
            ...
            <>
              ...
              <div className="px-6 py-4 border-b border-[#e2e8f0] bg-[#f8fafc] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="text-[#475569] border-[#e2e8f0] text-xs font-archivo capitalize"
                  >
                    {selectedResult.canonicalItem.canonicalType.replace(
                      "_",
                      " ",
                    )}
                  </Badge>
                ...
                <div className="px-6 py-5 space-y-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#0f172a] leading-relaxed font-data">
                      {editedStems.get(selectedResult.rowKey) ??
                        selectedResult.canonicalItem.stem ??
                        "(no stem)"}
                    </p>
                    {editedStems.has(selectedResult.rowKey) && (
                      <span className="text-[10px] text-[#111827] mt-1 block">
                        edited
                      </span>
                    )}
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {selectedResult.canonicalItem.choices.map((c, i) => {
                          ...
                          const isCorrect =
                            selectedResult.canonicalItem!.correctResponseIdentifiers.includes(
                              c.identifier,
                            );
                          return (
                            <div
                  ...
                  {selectedResult.canonicalItem.orderItems.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {selectedResult.canonicalItem.orderItems.map(
                          (item, i) => (
                            <div
                              key={i}
                              className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-[#f8fafc] border border-[#e2e8f0] text-sm text-[#0f172a]"
                            >
                    ...
                    >
                      ...
                      {selectedAudit.status === "ai_rejected" && (
                        <p className="text-xs text-destructive mt-1">
                          Click <strong>View Issues &amp; Fix</strong> above to
                          review and fix.
                        </p>
                      )}
                    </div>
                  ...
                  {!selectedAudit && !selectedLoading && (
                    <div className="rounded-lg border border-dashed border-[#e2e8f0] p-4 text-center">
                      <p className="text-sm text-[#64748b]">
                        Click <strong>Audit</strong> to analyse this question
                      </p>
                    </div>
                  )}
                </div>
          ...
          {modalResult?.canonicalItem && (
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-6 py-4 bg-[#f8fafc] border-b border-[#e2e8f0] space-y-3">
                ...
                {/* Original stem */}
                <div>
                  <p className="text-[10px] font-semibold text-[#475569] mb-1">
                    Original Stem
                  </p>
                  <p className="text-sm text-[#0f172a] leading-relaxed bg-card rounded-lg border border-[#e2e8f0] px-3 py-2">
                    {modalResult.canonicalItem.stem || (
                      <span className="text-[#64748b] italic">(no stem)</span>
                    )}
                  </p>
                </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {modalResult.canonicalItem.choices.map((c, i) => {
                        const alpha = "ABCDEFGHIJ"[i] ?? String(i + 1);
                        const isCorrect =
                          modalResult.canonicalItem!.correctResponseIdentifiers.includes(
                            c.identifier,
                          );
                        return (
                          <div
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {modalResult.canonicalItem.orderItems.map((item, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs bg-card border border-[#e2e8f0] text-[#0f172a]"
                        >
                ...
                {modalAudit?.issues && modalAudit.issues.length > 0 && (
                  <div>
                    <p className="text-[10px] font-bold text-[#64748b] uppercase tracking-wider mb-2">
                      {modalAudit.issues.length} Issue
                      {modalAudit.issues.length !== 1 ? "s" : ""} Detected
                    </p>
                    <div className="space-y-2">
                      {modalAudit.issues.map(
                        (issue: AuditIssue, idx: number) => {
                          const colors =
                            ISSUE_COLORS[issue.issue_type] ??
                            ISSUE_COLORS.clarity;
                          return (
                            <div
                              key={idx}
                            ...
                            >
                              <div className="flex items-start gap-3">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-sm text-[#0f172a] mt-0.5 leading-relaxed">
                                    {issue.description}
                                  </p>
                                  <p
                                    className={`text-xs mt-1.5 italic ${colors.text}`}
                                  >
                ...
                <div>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setEditedStems((prev) =>
                            new Map(prev).set(modalKey, fixed),
                          );
                        } catch (err: any) {
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      modalKey &&
                      setEditedStems((prev) =>
                        new Map(prev).set(modalKey, e.target.value),
                      )
                    }
                    rows={3}
