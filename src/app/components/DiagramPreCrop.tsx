...
export function DiagramPreCrop({
  imageUrl,
  onApprove,
  onCancel,
  allPages = [],
  ...
  const [popoverMode, setPopoverMode] = useState<
    "main" | "options" | "qnumber"
  >("main");

  // Pending tag state (held while user enters Q number)
  const [pendingType, setPendingType] = useState<"stem" | "option" | null>(
    null,
  );
  ...
  const updateCropTag = (
    ...
    setCrops((prev) =>
      prev.map((c) =>
        c.id === id ? { ...c, type, optionLabel: label, questionNumber } : c,
      ),
    );
    setSelectedCropId(null);
    setPopoverMode("main");
  ...
  const renderBox = (crop: CropBox, isCurrent = false) => {
    ...
    if (crop.type === "stem") {
      ...
      label = crop.questionNumber
        ? `Q${crop.questionNumber} Stem`
        : "Question Stem";
    } else if (crop.type === "option") {
      borderColor = "border-emerald-500";
      bgColor = "bg-emerald-500/20";
      label = crop.questionNumber
        ? `Q${crop.questionNumber} Opt ${crop.optionLabel}`
        : `Option ${crop.optionLabel}`;
    }
    ...
    return (
      ...
      >
        ...
        {!isCurrent && crop.type && (
          <div
            className={`absolute -top-6 left-[-2px] px-2 py-0.5 text-[10px] font-bold text-white uppercase tracking-wider rounded-t-sm whitespace-nowrap ${crop.type === "stem" ? "bg-blue-500" : "bg-emerald-500"}`}
            style={{
        ...
        {isSelected && !isCurrent && (
          <div
            ...
            style={{
              transform: `translate(-50%, 0) scale(${1 / zoomLevel})`,
            ...
            {popoverMode === "main" ? (
              <div className="flex gap-2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setPopoverMode("options")}
                  className="flex-1"
                >
                  Option →
                </Button>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-1.5">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") handleAssignQNumber();
                    }}
                    placeholder="Q no."
                    autoFocus
  ...
  const allTagged =
    crops.length > 0 && crops.every((c) => c.type && c.questionNumber);
              ...
              >
                {crops.length === 0 ? (
                  <span className="flex items-center">
                    <Check className="w-4 h-4 mr-2" /> Skip (No Diagrams)
                  </span>
                ) : (
                  <span className="flex items-center">
                    <Check className="w-4 h-4 mr-2" /> Approve & Extract
                  </span>
                )}
              </Button>
            </div>
