...
import { Question, McqQuestion, MsqQuestion, TextEntryQuestion } from '../core/questionTypes';
...
export function buildAuditPayload(row: QuestionRow): AiAuditPayload | null {
  ...
  if (q.type === 'MCQ') {
    ...
    payload.answer = q.correctAnswerId 
      ? q.options.find(o => o.id === q.correctAnswerId)?.label || q.correctAnswerId
      : null;
  } else if (q.type === 'MSQ') {
    payload.options = q.options.map(o => ({ label: o.label, text: o.text }));
    payload.answer = q.correctAnswerIds
 ...
 */
export async function simulateAiAudit(payload: AiAuditPayload): Promise<AiAuditResult> {
  // Simulate network delay (500ms - 1500ms)
  await new Promise(resolve => setTimeout(resolve, 500 + Math.random() * 1000));

  // Very basic mock logic for demonstration
  const textToCheck = `${payload.stem} ${payload.options?.map(o => o.text).join(' ')}`.toLowerCase();
  ...
  // Mock grammar issue
  if (textToCheck.includes('their is') || textToCheck.includes('there is') && payload.stem.includes('are')) {
    result.status = 'warning';
    result.categories.push('grammar');
    result.message = 'Possible grammatical error. Consider checking subject-verb agreement.';
  }
  ...
  if (payload.stem.length > 300 && !textToCheck.includes('passage')) {
    ...
    result.message = 'Stem is quite long. Consider breaking it down for better readability.';
  }

  // Mock logic issue (all of the above)
  if (payload.options?.some(o => o.text.toLowerCase().includes('all of the above'))) {
    result.status = 'warning';
    result.categories.push('formatting');
    result.message = '"All of the above" can be problematic if options are randomized in the LMS.';
  }
  ...
  if (textToCheck.includes('ambiguous')) {
    ...
    result.message = 'The phrasing is highly ambiguous and might confuse students.';
