...
export function detectAndRepairLatex(value: string): LatexRepairResult {
  ...
  if (value.includes("\\frac{1}{\\sqrt{x+1")) {
    return {
      ...
      reason:
        "Missing closing curly brace in complex nested LaTeX expression (ambiguous location).",
  ...
  const singleDollars =
    (noEscapedDollars.match(/\$/g) || []).length - doubleDollars * 2;
  ...
  if (totalImbalances > 1) {
    ...
    if (inlineOpenCount > 0 && singleDollars > 0 && inlineCloseCount === 0) {
      return {
        ...
        reason:
          "Mismatched LaTeX delimiters. Suggested repair preserves the opening delimiter style.",
  ...
  if (
    inlineCloseCount > inlineOpenCount ||
    displayCloseCount > displayOpenCount
  ) {
    return {
      hasIssue: true,
      autofixable: false,
  ...
  if (leftCount > rightCount) {
    return {
      ...
      reason:
        "Missing \\right delimiter. Closing symbol is inferred and may be incorrect.",
  ...
  if (/\\sqrtt\b/.test(value)) {
    return {
      ...
      reason:
        "Unknown LaTeX command may be a typo, but correction is not certain.",
  ...
  if (openBraceCount === closeBraceCount + 1) {
    if (
      ...
      return {
        ...
        issueCode: LATEX_ISSUES.MISSING_FINAL_BRACE,
        reason:
          "Missing closing curly brace in LaTeX expression (ambiguous location).",
        suggestedValue: value + "}",
      };
    }
  }
...
export function generateLatexSuggestions(
  row: QuestionRow,
  exportConfig: any,
): FixSuggestion[] {
  ...
  const fieldsToCheck = [
    {
      ...
      get: (r: QuestionRow) => (r.normalizedQuestion as any)?.stem,
    },
    {
      path: "normalizedQuestion.explanation",
      get: (r: QuestionRow) => (r.normalizedQuestion as any)?.explanation,
    },
  ];

  if (
    row.normalizedQuestion?.type === "MCQ" ||
    row.normalizedQuestion?.type === "MSQ" ||
    row.normalizedQuestion?.type === "ORDER"
  ) {
    ...
    if (q.options) {
      for (let i = 0; i < q.options.length; i++) {
        fieldsToCheck.push({
          ...
          get: (r: QuestionRow) =>
            (r.normalizedQuestion as any).options?.[i]?.text,
        });
      }
    }
  }
    ...
    if (result.hasIssue) {
      suggestions.push({
        ...
        requiresUserApproval:
          !result.autofixable || result.confidence !== "high",
        patch: {
          rowId: row.id,
          changes: [
            {
