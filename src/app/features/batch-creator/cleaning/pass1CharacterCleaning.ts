...
const NULL_TOKENS = new Set([
  'null', 'nil', 'none', 'n/a', 'na', '#n/a', '#null!', 'undefined', '-', '--', '—',
]);
...
const TRIMMABLE_FIELDS = new Set([
  'stem', 'explanation', 'correctAnswerId', 'rawStem',
]);
  ...
  if (cleaned.normalizedQuestion) {
    ...
    if ('options' in q && Array.isArray(q.options)) {
      for (let i = 0; i < q.options.length; i++) {
        ...
        if (r.changed) { opt.text = r.value; logs.push(...r.logs); }
      }
    }
    ...
    if ('acceptedAnswers' in q && Array.isArray(q.acceptedAnswers)) {
      for (let i = 0; i < q.acceptedAnswers.length; i++) {
        ...
        const r = cleanStringValue(q.acceptedAnswers[i], fieldName, row.id, true);
        if (r.changed) { q.acceptedAnswers[i] = r.value; logs.push(...r.logs); }
      }
    }
  }
...
function cleanStringValue(
  ...
  if (CRLF_RE.test(value)) {
    ...
    if (value !== before) {
      logs.push(makeLog(rowId, field, 'linebreak_normalization', before, value));
    }
  }
  ...
  for (const [pattern, replacement] of SMART_QUOTE_MAP) {
    if (pattern.test(value)) {
      ...
      if (value !== before) {
        logs.push(makeLog(rowId, field, 'smart_quote_normalization', before, value));
      }
    }
  }

  // (f) Trim leading/trailing whitespace for safe fields
  if (TRIMMABLE_FIELDS.has(field) || field.startsWith('options[') || field.startsWith('acceptedAnswers[')) {
    const before = value;
    value = value.trim();
    if (value !== before) {
