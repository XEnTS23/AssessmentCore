...
import { McqQuestion, MsqQuestion, TextEntryQuestion, Option } from '../core/questionTypes';
...
type SuggestionGenerator = (row: QuestionRow, issue: ValidationIssue) => FixSuggestion[];
 ...
 */
export function pass3GenerateSuggestions(rows: QuestionRow[], exportConfig?: any): SuggestionResult {
...
function makePatch(rowId: string, path: string, before: unknown, after: unknown): RowPatch {
...
register('MCQ_ANSWER_IN_OPTIONS', (row, issue) => {
  ...
  const labelMatch = q.options.find(o => o.label.toUpperCase() === upperAnswer);
  if (labelMatch) {
    suggestions.push(makeSuggestion(
      issue.id, issue.ruleId,
      `Match answer "${rawAnswer}" to option ${labelMatch.label} (case alignment)`,
      'high', true,
      makePatch(row.id, 'normalizedQuestion.correctAnswerId', rawAnswer, labelMatch.id),
    ));
    return suggestions;
  }

  // (b) Exact text match (case-insensitive)
  const exactTextMatches = q.options.filter(o =>
    o.text.trim().toLowerCase() === rawAnswer.trim().toLowerCase()
  );
  if (exactTextMatches.length === 1) {
    suggestions.push(makeSuggestion(
      ...
      'high', true,
      makePatch(row.id, 'normalizedQuestion.correctAnswerId', rawAnswer, exactTextMatches[0].id),
    ));
    return suggestions;
  }
  ...
  const fuzzyResults = q.options
    .map(o => ({ option: o, distance: levenshtein(rawAnswer.trim(), o.text.trim()) }))
    .filter(r => r.distance <= Math.max(2, Math.floor(rawAnswer.length * 0.3)))
  ...
  if (fuzzyResults.length === 1) {
    ...
    suggestions.push(makeSuggestion(
      ...
      'medium', false, // NEVER auto-apply fuzzy
      makePatch(row.id, 'normalizedQuestion.correctAnswerId', rawAnswer, best.option.id),
    ));
    ...
    for (const result of fuzzyResults.slice(0, 3)) {
      suggestions.push(makeSuggestion(
        ...
        'low', false,
        makePatch(row.id, 'normalizedQuestion.correctAnswerId', rawAnswer, result.option.id),
      ));
    }
  }
  ...
  if (suggestions.length === 0 && q.options.length > 0) {
    for (const opt of q.options) {
      suggestions.push(makeSuggestion(
        ...
        'low', false,
        makePatch(row.id, 'normalizedQuestion.correctAnswerId', rawAnswer, opt.id),
      ));
    }
  }
...
register('MSQ_CORRECT_ANSWERS_IN_OPTIONS', (row, issue) => {
  ...
  for (const badId of invalid) {
    ...
    const labelMatch = q.options.find(o => o.label.toUpperCase() === upperBad);
    if (labelMatch) {
      const newIds = q.correctAnswerIds.map(id => id === badId ? labelMatch.id : id);
      suggestions.push(makeSuggestion(
        issue.id, issue.ruleId,
        `Resolve "${badId}" to option ${labelMatch.label} (case alignment)`,
        'high', true,
        makePatch(row.id, 'normalizedQuestion.correctAnswerIds', q.correctAnswerIds, newIds),
      ));
      continue;
    }

    // Try exact text match
    const textMatches = q.options.filter(o =>
      o.text.trim().toLowerCase() === badId.trim().toLowerCase()
    );
    if (textMatches.length === 1) {
      const newIds = q.correctAnswerIds.map(id => id === badId ? textMatches[0].id : id);
      suggestions.push(makeSuggestion(
        issue.id, issue.ruleId,
        `Match answer text "${badId}" to option ${textMatches[0].label}`,
        'high', true,
        makePatch(row.id, 'normalizedQuestion.correctAnswerIds', q.correctAnswerIds, newIds),
      ));
    }
      ...
      changes: [
        { path: 'normalizedQuestion.type', before: 'MCQ', after: 'MSQ' },
        {
          ...
          after: q.correctAnswerId
            ? q.correctAnswerId.split(/[,;|]/).map(s => s.trim()).filter(Boolean)
            : [],
        },
      ],
    },
...
register('MCQ_SINGLE_CORRECT_ONLY', (row, issue) => {
  ...
  const parts = q.correctAnswerId.split(/[,;|]/).map(s => s.trim()).filter(Boolean);
  ...
  suggestions.push(makeSuggestion(
    ...
    'low', false,
    makePatch(row.id, 'normalizedQuestion.correctAnswerId', q.correctAnswerId, parts[0]),
  ));
...
register('MSQ_NO_DUPLICATE_CORRECT_ANSWERS', (row, issue) => {
  ...
  return [makeSuggestion(
    ...
    'high', true,
    makePatch(row.id, 'normalizedQuestion.correctAnswerIds', q.correctAnswerIds, deduped),
  )];
});
...
register('TEXT_ENTRY_NUMERIC_ANSWER_VALID', (row, issue) => {
  ...
  for (let i = 0; i < q.acceptedAnswers.length; i++) {
    ...
    if (isNaN(num)) {
      ...
      const cleaned = ans.replace(/,/g, '').replace(/[^0-9.\-+eE]/g, '').trim();
      ...
      if (!isNaN(cleanedNum) && cleaned !== '') {
        ...
        suggestions.push(makeSuggestion(
          ...
          'medium', false,
          makePatch(row.id, 'normalizedQuestion.acceptedAnswers', q.acceptedAnswers, newAnswers),
        ));
      }
    }
  }
