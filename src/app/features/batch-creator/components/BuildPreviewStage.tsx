...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${colors[type] ?? "bg-muted text-muted-foreground"}`}
    >
      {type}
    </span>
  );
...
export function BuildPreviewStage({
  wizard,
  upload,
}: {
  wizard: any;
  ...
  const savedConfig: ExportConfig =
    wizard.__exportConfig ?? DEFAULT_EXPORT_CONFIG;
  ...
  const mapping: ColumnMapping = useMemo(() => {
    ...
    const availableColumns = Object.keys(rawRows[0] || {}).filter(
      (c: string) => !c.startsWith("__"),
    );
    return inferColumnMapping(availableColumns);
  }, [upload?.output?.mapping, upload?.output?.rawRows]);

  const sourceRows: QuestionRow[] = useMemo(() => {
    if (wizard.__processedRows && wizard.__processedRows.length > 0) {
      return wizard.__processedRows;
    }
    if (!upload?.output?.rawRows) return [];
    return upload.output.rawRows.map((r: any) => normalizeRow(r, mapping));
  }, [wizard.__processedRows, upload?.output?.rawRows, mapping]);

  const stage = useExportBuildStage(sourceRows, savedConfig);

  // Auto-run initial build on mount when rows are loaded
  useEffect(() => {
    if (stage.buildStatus === "idle" && sourceRows.length > 0) {
      stage.runBuild(savedConfig);
    }
  }, [stage.buildStatus, sourceRows.length, savedConfig, stage]);

  // Mark wizard stage complete once download is ready
  useEffect(() => {
    wizard.__mockSetComplete(
      "BUILD_PREVIEW",
      stage.packageResult?.isDownloadReady ?? false,
    );
          ...
          <>
            ...
            <div className="flex items-center gap-2 rounded-full border bg-muted/40 px-3 py-1 text-xs text-muted-foreground mr-2">
              <span className="font-medium text-foreground">
                {TARGET_LABELS[savedConfig.target] ?? savedConfig.target}
              </span>
              <span>·</span>
            ...
            <Button
              size="sm"
              variant={
                stage.packageResult?.isDownloadReady ? "default" : "outline"
              }
              className="h-8 gap-1.5 text-xs"
            ...
            >
              ...
              {stage.packageResult?.downloadFileName
                ? stage.packageResult.downloadFileName.endsWith(".zip")
                  ? "Download ZIP"
                  : "Download"
                : "Download"}
            </Button>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
            ...
            {stage.previews.length === 0 ? (
              ...
              stage.previews.map((p, i) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 border-b transition-colors flex items-start gap-2 ${
                    i === stage.selectedPreviewIndex
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-foreground truncate leading-snug">
                      {p.stem || "—"}
                    </p>
                  </div>
                </button>
        ...
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden border-r">
          ...
          <div className="flex items-center border-b px-4 gap-3 shrink-0 h-10">
            ...
            >
              <FileCode2 className="h-3.5 w-3.5 inline mr-1" />
              Raw Output
            </button>
          </div>
          ...
          {stage.activeTab === "student" ? (
            <div className="flex-1 overflow-hidden">
              {stage.buildStatus === "building" ? (
                <EmptyState
                  ...
                  text="Building question artifacts and preview..."
                />
                ...
                stage.studentPreviewHtml ? (
                  ...
                  <EmptyState
                    ...
                    text="Select a question to preview."
                  />
                )
                ...
                <div className="p-6 overflow-y-auto h-full space-y-4">
                  <div className="p-4 border border-destructive/30 bg-destructive/5 rounded-lg space-y-2">
                    <h4 className="text-destructive font-semibold text-xs uppercase flex items-center gap-1.5">
                      <AlertTriangle className="h-4 w-4" /> Build Blocked
                    </h4>
                    <p className="text-xs text-destructive/90">
                      The assessment export could not be generated. Please fix the following issue(s):
                    </p>
                    <ul className="list-disc pl-5 text-destructive/90 text-xs space-y-1">
                      {stage.packageResult?.validationErrors.map((err, i) => (
                        <li key={`err-${i}`}>{err.message}</li>
                      ))}
                      {(!stage.packageResult?.validationErrors ||
                        stage.packageResult.validationErrors.length === 0) && (
                        <li>An unknown error occurred during build.</li>
                      )}
                    </ul>
                  </div>
                </div>
                ...
                <EmptyState
                  ...
                  text="Click Build to generate the artifacts. The preview will be available once the build succeeds."
                />
              )}
            </div>
          ) : (
            <div className="flex-1 overflow-hidden flex flex-col">
              <div className="flex-1 overflow-auto bg-[#0d1117] text-[#c9d1d9] font-mono text-xs p-4 leading-relaxed relative">
                ...
                {stage.buildStatus === "success" &&
                  stage.selectedArtifact &&
                  stage.selectedArtifact.fileName.endsWith(".xml") && (
                ...
                {stage.buildStatus === "idle" && (
                  <p className="text-[#8b949e]">
                    Click <strong>Build</strong> to generate artifacts.
                  </p>
                )}
                {stage.buildStatus === "building" && (
                ...
                stage.selectedArtifact ? (
                  isEditingRaw ? (
                    ...
                    <pre className="whitespace-pre-wrap break-all">
                      {stage.selectedArtifact.data}
                    </pre>
                  )
                ) : stage.buildStatus === "error" && !stage.selectedArtifact ? (
                  <p className="text-red-400">
                    Build failed. See error panel below.
                  </p>
                ) : null}
              </div>
            </div>
...
function StatusCard({ buildStatus }: { buildStatus: BuildStatus }) {
  ...
  return (
    ...
    >
      {icons[buildStatus]}
      <span className={`text-xs font-medium ${STATUS_COLORS[buildStatus]}`}>
        {labels[buildStatus]}
      </span>
    </div>
  );
  ...
  return (
    <div className="rounded-lg border bg-card p-3 space-y-2 text-xs">
      ...
      {preview.optionLines.length > 0 && (
        <ul className="space-y-1 mt-1">
          {preview.optionLines.map((o) => (
            <li
              key={o.label}
              className={`flex items-start gap-1.5 ${o.isCorrect ? "text-success font-medium" : "text-muted-foreground"}`}
            >
      ...
      <div className="flex items-center justify-between text-[10px] text-muted-foreground pt-1 border-t">
        <span>
          Answer:{" "}
          <strong className="text-foreground">{preview.answerSummary}</strong>
        </span>
        <span>
