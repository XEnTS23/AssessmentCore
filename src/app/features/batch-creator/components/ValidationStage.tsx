...
export function ValidationStage({
  ...
  const v = useValidationStage(upload.output?.rawRows, upload.output?.mapping);
      ...
      <div className="flex h-full items-center justify-center p-8">
        <div className="w-full max-w-md space-y-4 rounded-lg border bg-card p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="font-semibold text-foreground">{phaseLabel}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {v.progress.processed.toLocaleString()} of{" "}
                {v.progress.total.toLocaleString()} rows
              </p>
            </div>
          ...
          <Progress value={v.progress.percent} className="h-2" />
          <p className="text-xs text-muted-foreground">
            Leaving this stage or replacing the upload safely cancels this run.
          </p>
        </div>
      </div>
    );
  ...
  if (v.processingError) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          Return to Upload, correct or replace the file, and try again.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col bg-background text-sm">
      ...
      <div className="flex gap-4 p-4 border-b">
        ...
        <SummaryCard
          title="Caution"
          value={v.summary.caution}
          variant="warning"
        />
        <SummaryCard
          title="Rejected"
          value={v.summary.rejected}
          variant="destructive"
        />
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {v.ruleFrequencies.map((rule) => (
              <button
                ...
                onClick={() =>
                  v.setSelectedRuleId(
                    rule.ruleId === v.selectedRuleId ? "all" : rule.ruleId,
                  )
                }
              ...
              >
                <span className="truncate pr-2" title={rule.message}>{rule.message}</span>
          ...
          <div className="flex h-14 px-4 items-center justify-between border-b">
            <div className="flex items-center gap-4">
              ...
              <div className="flex rounded-md border overflow-hidden text-[9px] scale-90 origin-left">
                {(
                  ["all", "valid", "caution", "rejected"] as FilterStatus[]
                ).map((status) => (
                  <button
                    key={status}
                    onClick={() => v.setFilterStatus(status)}
                    className={`px-2.5 py-1 capitalize transition-colors ${
                      v.filterStatus === status
              ...
              <tbody className="divide-y">
                {paginatedRows.length === 0 ? (
                  ...
                  paginatedRows.map((row) => {
                    ...
                    const hasImageError =
                      row.mediaReferences?.some((m) => m.status === "failed") ??
                      false;
                    const stemText =
                      (q && "stem" in q
                        ? q.stem
                    ...
                    return (
                      <tr
                        key={row.id}
                        className={`${rowClass} transition-colors hover:opacity-90`}
                      >
                        <td className="px-4 py-3">{row.sourceRowNumber}</td>
                        <td className="px-4 py-3">
                          {/* FIX 3: Safely check if any issue is a media category */}
                          {hasImageError ||
                          safeIssues.some((i) => i.category === "media") ? (
                            <div className="h-2 w-2 rounded-full bg-destructive mx-auto"></div>
                          ) : (
                            <span className="text-muted-foreground/30 mx-auto block w-max">
                              —
                            </span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-muted-foreground">
                          row_{row.sourceRowNumber - 1}
                        </td>
                        ...
                        <td className="px-4 py-3 text-muted-foreground truncate max-w-[100px]">
                          {q && "correctAnswerId" in q
                            ? (() => {
                                const opt = q.options?.find(
                                  (o: any) => o.id === q.correctAnswerId,
                                );
                                return opt
                                  ? opt.label || opt.text
                                  : q.correctAnswerId || "—";
                              })()
                            : q && "correctAnswerIds" in q
                              ? (q as any).correctAnswerIds
                                  .map((id: string) => {
                                    const opt = (q as any).options?.find(
                                      (o: any) => o.id === id,
                                    );
                                    return opt ? opt.label || opt.text : id;
                                  })
                                  .join(", ") || "—"
                              : q && "correctSequenceIds" in q
                                ? (q as any).correctSequenceIds
                                    .map((id: string) => {
                                      const opt = (q as any).options?.find(
                                        (o: any) => o.id === id,
                                      );
                                    ...
                                    .join(" → ") || "—"
                                : q && "acceptedAnswers" in q
                                  ? (q as any).acceptedAnswers?.join(", ") ||
                                    "—"
                                  : "—"}
                        </td>
          ...
          <div className="flex shrink-0 items-center justify-between border-t border-border bg-muted/20 px-4 py-2">
            <div className="text-xs text-muted-foreground">
              ...
              {Math.min(
                ...
                v.filteredRows?.length || 0,
              )}{" "}
              to {Math.min(currentPage * pageSize, v.filteredRows?.length || 0)}{" "}
              of {v.filteredRows?.length || 0}
            </div>
...
function SummaryCard({
  ...
  return (
    <div
      className={`flex min-w-[120px] flex-col justify-center rounded-md border ${borderColors[variant]} px-4 py-2`}
    >
      <span className="text-xs text-muted-foreground mb-1">{title}</span>
      <span className={`text-xl font-medium ${textColors[variant]}`}>
        {value}
      </span>
    </div>
  );
...
function IssueBadge({
  ...
  if (severity === "warning" || severity === "review")
    color = "border-warning/40 text-warning bg-warning/5";

  return (
    <span
      className={`rounded border px-1.5 py-0.5 text-[10px] truncate max-w-[250px] inline-block align-bottom ${color}`}
      title={message}
    >
      {message}
    </span>
