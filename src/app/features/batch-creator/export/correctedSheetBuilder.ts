...
function extractOptions(row: QuestionRow): {
  a?: string;
  b?: string;
  c?: string;
  d?: string;
  ...
  if (!q || (q.type !== "MCQ" && q.type !== "MSQ" && q.type !== "ORDER"))
    return {};
  const opts: Record<string, string> = {};
  for (const opt of (q as McqQuestion | MsqQuestion | OrderQuestion).options) {
...
function extractCorrectAnswer(row: QuestionRow): string {
  ...
  if (q.type === "MSQ")
    return (q as MsqQuestion).correctAnswerIds?.join("; ") ?? "";
  if (q.type === "TEXT_ENTRY")
    return (q as TextEntryQuestion).acceptedAnswers?.join(" | ") ?? "";
  if (q.type === "ORDER")
    return (q as OrderQuestion).correctSequenceIds?.join("; ") ?? "";
  return "";
}

function serializeMediaReferences(row: QuestionRow): string {
  return JSON.stringify(
    (row.mediaReferences || []).map((reference) => ({
      type: reference.contentType || "image",
      role: reference.role,
      url: reference.publicUrlSource,
...
export function buildCorrectedSheet(
  ...
  const dataRows = rows.map((row) => {
    ...
    const cells: unknown[] = [
      ...
      explanation,
      JSON.stringify(row.manualFixSections || []),
      serializeMediaReferences(row),
      row.mathReferences?.length ?? 0,
      row.timeLimitConfig?.timeLimitSeconds ?? "",
      meta.subject ?? "",
      meta.chapter ?? "",
      meta.topic ?? "",
      meta.difficulty ?? "",
    ...
    if (includeScoring) {
      cells.push(row.scoringConfig?.marks ?? "", meta.negativeMarks ?? "");
    }

    if (includeIssues) {
      cells.push(row.issues?.length ?? 0, serializeIssues(row));
    }
