...
import { AiAuditResult, buildAuditPayload, simulateAiAudit } from '../audit/aiAuditEngine';
...
export function useAiAuditStage(inputRows: QuestionRow[] = []) {
  ...
  const [auditResults, setAuditResults] = useState<Record<string, AiAuditResult>>({});
  ...
  const handleAction = useCallback((rowId: string, action: 'ignored' | 'accepted') => {
    ...
    if (action === 'accepted') {
      ...
      if (currentResult?.suggestedFix) {
        ...
        setRows(prevRows => {
          ...
          const patchResult = applyPatch(row, currentResult.suggestedFix!, engine, {
            allRows: prevRows,
