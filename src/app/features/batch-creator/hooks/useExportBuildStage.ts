...
import {
  renderAllPreviews,
  QuestionPreview,
} from "../preview/questionPreviewRenderer";
...
export function useExportBuildStage(
  ...
  const studentPreviewHtml = useMemo(() => {
    ...
    let tempRow = row;
    const currentXml = packageResult?.validatedArtifacts?.find(
      (a) =>
        row.metadata?.questionId &&
        a.fileName.includes(row.metadata.questionId),
    )?.data;

    if (
      typeof currentXml === "string" &&
      currentXml.includes("<assessmentItem")
    ) {
      try {
        const parser = new DOMParser();
        const doc = parser.parseFromString(currentXml, "text/xml");
        const promptNode =
          doc.querySelector("prompt") || doc.querySelector("itemBody > p");
        ...
        if (
          ...
          if (mapEntry && mapEntry.hasAttribute("mappedValue")) {
            const parsedMarks = parseFloat(
              mapEntry.getAttribute("mappedValue") || "1",
            );
          ...
          if (tempRow.normalizedQuestion) {
            if (promptNode) {
              tempRow.normalizedQuestion.stem =
                promptNode.innerHTML.trim() || promptNode.textContent || "";
            }
            if (
              choices.length > 0 &&
              tempRow.normalizedQuestion &&
              "options" in tempRow.normalizedQuestion &&
              tempRow.normalizedQuestion.options
            ) {
              Array.from(choices).forEach((choice) => {
                const id = choice.getAttribute("identifier");
                const option = (
                  tempRow.normalizedQuestion as any
                ).options?.find((o: any) => o.id === id);
                if (option) {
                  option.text =
                    choice.innerHTML.trim() || choice.textContent || "";
                }
              });
            }
            if (correctValues.length > 0) {
              const vals = Array.from(correctValues).map(
                (v) => v.textContent || "",
              );
    ...
    config,
    refreshKey,
    packageResult?.validatedArtifacts,
  ]);
      ...
      try {
        ...
        setPackageResult({
          buildResult: {
            ...
            warnings: [],
            errors: [
              { code: "BUILD_EXCEPTION", message: e?.message ?? String(e) },
            ],
          },
          validatedArtifacts: [],
          validationErrors: [
    ...
    (index: number) => {
      ...
      if (packageResult?.validatedArtifacts) {
        ...
        if (row && row.metadata?.questionId) {
          ...
          const artifactIndex = packageResult.validatedArtifacts.findIndex(
            (a) => a.fileName.includes(qid),
          );
          if (artifactIndex !== -1) {
  ...
  const selectArtifact = useCallback(
    (index: number) => setSelectedArtifactIndex(index),
    [],
  );
