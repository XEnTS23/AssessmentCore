...
export function tokenizeRichContent(text: string | undefined | null): RichContent {
  if (!text) {
    return { raw: '', tokens: [] };
  }
  ...
  while (currentIndex < raw.length) {
    ...
    for (const delim of DELIMITERS) {
      ...
      if (idx !== -1) {
        ...
        } else if (idx === nextDelimiterIndex && nextDelimiter && delim.open.length > nextDelimiter.open.length) {
    ...
    if (nextDelimiterIndex > currentIndex) {
      tokens.push({ type: 'text', value: raw.slice(currentIndex, nextDelimiterIndex) });
    }
  ...
  for (const token of tokens) {
    if (token.type === 'text' && mergedTokens.length > 0 && mergedTokens[mergedTokens.length - 1].type === 'text') {
      const last = mergedTokens[mergedTokens.length - 1] as Extract<ContentToken, { type: 'text' }>;
