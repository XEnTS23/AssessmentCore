...
import { readRawAnswer, resolveAnswerTokens } from "./answerResolution";
      ...
      if (!opt.text || opt.text.trim() === "") {
        issues.push(
          createIssue(
            this,
            row.id,
            `Option ${opt.label || idx + 1} has empty text.`,
            `options[${idx}]`,
          ),
        );
      }
      ...
      if (!opt.id || opt.id.trim() === "") {
        return [
          createIssue(
            this,
            row.id,
            `Option is missing an internal identifier.`,
            "options",
          ),
        ];
      }
...
export const MCQ_HAS_CORRECT_ANSWER: ValidationRule = {
  ...
  validate(row) {
    const q = row.normalizedQuestion as McqQuestion;
    if (
      q.correctAnswerId === undefined ||
      q.correctAnswerId === null ||
      String(q.correctAnswerId).trim() === ""
    ) {
      return [
        createIssue(
          this,
  ...
  validate(row) {
    ...
    return [
      createIssue(
        this,
        row.id,
        `Correct answer '${q.correctAnswerId}' does not match any option.`,
        "correctAnswerId",
      ),
    ];
  },
...
export const MCQ_ANSWER_TEXT_MATCH: ValidationRule = {
  ...
  validate(row, context) {
    ...
    const resolution = resolveAnswerTokens([rawAnswer], q.options)[0];
    if (resolution.kind !== "unmatched") return [];

    return [
      createIssue(
        this,
        row.id,
        `Answer text '${rawAnswer}' does not match any option.`,
        "correctAnswerId",
        { rawAnswer },
      ),
    ];
  ...
  validate(row, context) {
    ...
    return [
      createIssue(
        this,
        row.id,
        `Answer text '${rawAnswer}' matches multiple options. Select an unambiguous option identifier.`,
        "correctAnswerId",
        {
          rawAnswer,
          matchingOptionIds: resolution.matchingOptionIds,
        },
      ),
    ];
  },
      ...
      return [
        createIssue(
          ...
          [
            {
              id: "convert_to_msq",
              description: "Change type to MSQ",
              patch: { "normalizedQuestion.type": "MSQ" },
            },
          ],
        ),
      ];
