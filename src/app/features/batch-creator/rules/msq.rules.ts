    ...
    if (!q.options || q.options.length < 2) {
      return [
        createIssue(
          this,
          row.id,
          "MSQ must have at least 2 options.",
          "options",
        ),
      ];
    }
    ...
    if (!q.correctAnswerIds || q.correctAnswerIds.length === 0) {
      return [
        createIssue(
          this,
          row.id,
          "At least one correct answer is required for MSQ.",
          "correctAnswerIds",
        ),
      ];
    }
      ...
      if (!ans || ans.trim() === "") {
        return [
          createIssue(
            this,
            row.id,
            `Correct answer contains an empty identifier.`,
            "correctAnswerIds",
          ),
        ];
      }
      ...
      if (seenIds.has(opt.id)) {
        return [
          createIssue(
            this,
            row.id,
            `Duplicate option identifier detected internally.`,
            "options",
          ),
        ];
      }
...
export const MSQ_CORRECT_ANSWERS_IN_OPTIONS: ValidationRule = {
  ...
  validate(row) {
    ...
    const invalidAnswers = q.correctAnswerIds.filter(
      (ans) => !optionIds.has(ans),
    );
...
export const MSQ_MIXED_IDENTIFIER_MODE: ValidationRule = {
  ...
  validate(row, context) {
    const q = row.normalizedQuestion as MsqQuestion;
    const rawAnswer = readRawAnswer(row, context);
    if (!rawAnswer || !q.options?.length) return [];

    const resolutions = resolveAnswerTokens(
      splitMsqAnswer(rawAnswer),
      q.options,
    );
    const hasDirect = resolutions.some((result) => result.kind === "direct");
    const hasText = resolutions.some((result) => result.kind === "text");
    if (!hasDirect || !hasText) return [];

    return [
      createIssue(
        this,
        row.id,
        "Answers mix option identifiers and option text. Use one answer mode consistently.",
        "correctAnswerIds",
        {
          rawAnswer,
          directTokens: resolutions
            .filter((result) => result.kind === "direct")
            .map((result) => result.token),
          textTokens: resolutions
...
export const MSQ_ANSWER_TEXT_MATCH: ValidationRule = {
  ...
  validate(row, context) {
    ...
    const unmatched = resolutions.filter(
      (result) => result.kind === "unmatched",
    );
    ...
    return [
      createIssue(
        this,
        row.id,
        `Answer text not found in options: ${unmatched.map((result) => result.token).join(", ")}.`,
        "correctAnswerIds",
        {
          rawAnswer,
          unmatchedTokens: unmatched.map((result) => result.token),
        },
      ),
    ];
  },
...
export const MSQ_ANSWER_TEXT_AMBIGUOUS: ValidationRule = {
  ...
  validate(row, context) {
    ...
    const ambiguous = resolveAnswerTokens(
      splitMsqAnswer(rawAnswer),
      q.options,
    ).filter((result) => result.kind === "ambiguous");
    ...
    return [
      createIssue(
        this,
        row.id,
        `Answer text matches multiple options: ${ambiguous.map((result) => result.token).join(", ")}.`,
        "correctAnswerIds",
        {
          rawAnswer,
          ambiguousTokens: ambiguous.map((result) => ({
            token: result.token,
            matchingOptionIds: result.matchingOptionIds,
          })),
        },
      ),
    ];
      ...
      return [
        createIssue(
          ...
          [
            {
              id: "dedup_answers",
              description: "Remove duplicate answers",
              patch: {
  ...
  validate(row, context) {
    ...
    const duplicates = context.allRows.filter((r) => {
      ...
      if (
        ...
        otherQ.type !== "MSQ" ||
        !otherQ.stem ||
        !otherQ.correctAnswerIds
      )
        return false;

      return (
        otherQ.stem.trim().toLowerCase() === stemClean &&
    ...
    if (duplicates.length > 0) {
      return [
        createIssue(
          this,
          row.id,
          "This MSQ has identical stem and exact same answer set as another question.",
          "stem",
        ),
      ];
    }
