...
export const ORDER_MIN_OPTIONS: ValidationRule = {
  ...
  validate(row) {
    ...
    if (!q.options || q.options.length < 2) {
      return [createIssue(this, row.id, 'Ordering question must have at least 2 options to order.', 'options')];
    }
    return [];
  }
};
...
export const ORDER_HAS_CORRECT_SEQUENCE: ValidationRule = {
  ...
  validate(row) {
    ...
    if (!q.correctSequenceIds || q.correctSequenceIds.length === 0) {
      return [createIssue(this, row.id, 'Correct sequence is required.', 'correctSequenceIds')];
    }
