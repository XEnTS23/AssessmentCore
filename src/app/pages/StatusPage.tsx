...
import { Activity, CheckCircle2, CircleAlert, Server, Facebook, Github, Linkedin, Mail, MapPin, MessageCircle, Phone, Youtube } from "lucide-react";
import { useTheme } from "../../contexts/ThemeContext";
import { Navbar } from "../components/Navbar";

const services = [
  { name: "OCR ingestion", status: "Operational", icon: CheckCircle2, tone: "text-emerald-500" },
  { name: "Validation engine", status: "Operational", icon: CheckCircle2, tone: "text-emerald-500" },
  { name: "QTI export", status: "Operational", icon: CheckCircle2, tone: "text-emerald-500" },
  { name: "Support desk", status: "Monitoring", icon: CircleAlert, tone: "text-amber-500" },
];
    ...
    <div className="min-h-screen bg-background text-foreground transition-colors duration-200">
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-16 pt-20 text-center">
        ...
        <h1 className="max-w-4xl mx-auto text-5xl font-semibold tracking-tight leading-[1.1] sm:text-6xl">
          Platform status for the <span className="text-primary">deterministic pipeline.</span>
        </h1>
        <p className="mt-6 mx-auto max-w-2xl text-lg leading-relaxed text-muted-foreground">
          Current service health and operational notes for OCR, validation, and export workflows.
        </p>
      </section>
      ...
      <section className="border-t border-border bg-muted/30 px-6 py-20 transition-colors duration-200">
        <div className="mx-auto grid w-full max-w-6xl gap-6 lg:grid-cols-[1.2fr_1fr]">
          <div className="rounded-[0.625rem] border border-border bg-card p-8 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-2xl font-semibold tracking-tight text-foreground">All systems operational</h2>
              </div>
            </div>
          ...
          <div className="rounded-[0.625rem] border border-border bg-card p-8 shadow-sm">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">System notes</div>
            <ul className="mt-4 space-y-3 text-sm text-muted-foreground">
              ...
              <li>QTI packages continue to export with full LMS compatibility.</li>
              <li>No active incidents reported.</li>
            </ul>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/contact" className="inline-flex h-11 items-center gap-2 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
                Contact support
              </Link>
