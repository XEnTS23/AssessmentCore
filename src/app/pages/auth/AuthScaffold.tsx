...
function LogRow({ label, value, tone }: { label: string; value: string; tone?: "ok" | "warn" | "base" }) {
  const color = tone === "ok" ? "text-success" : tone === "warn" ? "text-warning" : "text-chart-1";
  return (
    ...
    <div className="grid min-h-screen grid-cols-1 bg-background lg:grid-cols-2">
      <section className="flex flex-col border-r border-border bg-card px-7 py-8 sm:px-12">
        <Link to="/" className="inline-flex items-center gap-2">
          <img src={isDark ? '/logo-dark-1.png' : '/AC_logo.png'} alt="AssessmentCore logo" className="h-8 w-8 rounded-md object-contain" />
      ...
      <section className="relative hidden overflow-hidden bg-muted/30 lg:flex lg:items-center lg:justify-center lg:p-10">
        ...
        <div className="relative w-full max-w-lg rounded-xl border border-border bg-card shadow-xl">
          <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
            ...
            <span className="font-mono text-[11px] text-muted-foreground">validation.log</span>
          </div>
          <div className="space-y-0.5 p-4 font-mono">
            <LogRow label="parse" value="questions.xlsx · 2418 rows" tone="base" />
            <LogRow label="detect" value="13 columns · confidence high" tone="base" />
            <LogRow label="validate" value="pass 2341 · caution 54 · reject 23" tone="ok" />
            <LogRow label="duplicate" value="11 exact · 3 near · 0 conflict" tone="warn" />
            <LogRow label="clean" value="pass-1 · 1204 rows normalized" tone="base" />
            <LogRow label="clean" value="pass-2 · 48 answers aligned" tone="base" />
            <LogRow label="audit" value="2395 certified · 23 flagged" tone="ok" />
