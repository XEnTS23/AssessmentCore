...
function isCorrectFeedbackIdentifier(identifier: string): boolean {
  const normalized = identifier.toLowerCase();
  return (
    !isIncorrectFeedbackIdentifier(normalized) && normalized.includes("correct")
  );
}
...
function detectQtiVersion(doc: Document): string {
  ...
  if (namespace.includes("v3p0") || namespace.includes("imsqtiasi_v3p0")) {
    ...
    namespace.includes("v2p1") ||
    namespace.includes("imsqtiasi_v2p1")
  ) {
    return "2.1";
  } else if (
    namespace.includes("v1p2") ||
    namespace.includes("imsqtiasi_v1p2")
  ) {
    return "1.2";
  }
  return "unknown";
...
export function extractTemplateFields(templateXml: string): ExtractedTemplate {
  ...
  if (rootLocalName !== "assessmentItem" && rootLocalName !== "item") {
    throw new Error(
      `Root element is <${rootLocalName}>, expected <assessmentItem> or <item>`,
    );
  }
  ...
  if (itemBody) {
    ...
    if (choiceInteraction) {
      ...
      const simpleChoices = findAllByLocalName(
        choiceInteraction,
        "simpleChoice",
      );
      simpleChoices.forEach((choice, index) => {
        const identifier =
          ...
          `choice_${String.fromCharCode(65 + index)}`;
        addField(
    ...
    if (correctResponse) {
      ...
      if (values.length > 0) {
        addField(
          ...
          "identifier",
          true,
          "Identifier of correct choice(s)",
        );
      }
    }
  }
