...
function detectQuestionType(row: Record<string, any>): string {
  const explicit =
    ...
    getFirstByKeyPattern(
      row,
      /question[_\s-]*type|interaction[_\s-]*type|^type$/i,
    );
...
function buildStructuralFallbackMessages(row: Record<string, any>): string[] {
  ...
  if (!textEntryLike) {
    if (options.length === 0)
      messages.push("No options were detected for this row.");
    if (options.length > 0 && options.length < 2)
      messages.push(
        "Only one option is present; at least two are required for choice-based questions.",
      );
  }
  ...
  const answer =
    ...
    getFirstByKeyPattern(
      row,
      /correct[_\s-]*answer|^answer$|expected[_\s-]*answer|response/i,
    );
...
export function mapRowToAuditPayload(
  row: Record<string, any>,
): AuditRowPayload {
  const questionType = detectQuestionType(row);
  ...
  if (!isTextEntryLikeType(questionType)) {
    ...
    if (options.length === 0) {
      const optionCandidates = Object.entries(row)
        .filter(
          ([k, v]) =>
            /option|choice|^ans[_\s-]?[a-j]$/i.test(k) &&
            String(v ?? "").trim(),
        )
        .map(([, v]) => String(v).trim());
...
function mapRowToEdgePayload(row: Record<string, any>) {
  ...
  const correctResponseIdentifiers =
    tokens.length > 0
      ? tokens.map(normalizeChoiceIdentifier)
      : payload.answer
        ? [normalizeChoiceIdentifier(payload.answer)]
        : [];

  let inferredQuestionType = detectQuestionType(row) || "unknown";
  if (
    !inferredQuestionType ||
    inferredQuestionType === "unknown" ||
    inferredQuestionType === "—" ||
    inferredQuestionType === "-"
  ) {
    const hasChoices = choices.length > 0;
    const answerLooksLikeChoiceLabel = /^[A-J](\s*[,;|]\s*[A-J])*$/i.test(
      payload.answer || "",
    );
    inferredQuestionType =
      hasChoices || answerLooksLikeChoiceLabel ? "mcq" : "textentry";
  }
...
export async function runAiAudit(
  ...
  try {
    ...
    const normalizedIssues = (edgeResult.issues ?? []).filter(
    ...
    const finalExplanation = isCertified
      ...
      : finalIssues[0]?.description ||
        "AI marked this question as failed but did not return detailed reasons.";
  ...
  try {
    if (
      ...
      return {
        available: false,
        error:
          "Supabase env is missing (VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY).",
      };
    }
