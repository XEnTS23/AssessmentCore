...
export class ExportService {
  ...
  public static async generateExport(
    ...
    try {
      ...
      throw new Error('Export generation failed. Please check the data format.');
    }
  }
   ...
   */
  private static async buildZipExport(data: any[], config: ExportConfiguration): Promise<Blob> {
    const zip = new JSZip();
  ...
  private static buildStandaloneXml(row: any, config: ExportConfiguration): string {
  ...
  private static async exportMediaToFolder(data: any[], folder: JSZip | null): Promise<void> {
