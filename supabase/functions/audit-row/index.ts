...
function inferStructuralIssues(
  ...
  const normalizedAnswers = Array.isArray(correctResponseIdentifiers)
    ? correctResponseIdentifiers.map((a) => String(a ?? '').trim()).filter(Boolean)
    : [];
  ...
  if (!isTextEntryLike && normalizedChoices.length === 0) {
    issues.push({
      ...
      suggestion: 'Provide valid options (for MCQ/MSQ) or switch to an open-ended question type.',
    });
  }

  if (!isTextEntryLike && normalizedChoices.length > 0 && normalizedChoices.length < 2) {
    issues.push({
      issue_type: 'logic',
      description: 'Only one option was found; this is structurally invalid for choice-based questions.',
      suggestion: 'Add at least two options so the answer can be evaluated correctly.',
    });
  }
  ...
  if (!isTextEntryLike && normalizedChoices.length > 0 && normalizedAnswers.length > 0) {
    const choiceLabels = new Set(normalizedChoices.map((_, idx) => String.fromCharCode(65 + idx)));
    const unmatched = normalizedAnswers.filter((ans) => {
      const upper = ans.toUpperCase();
      if (choiceLabels.has(upper)) return false;
      return !normalizedChoices.some((opt) => opt.toLowerCase() === ans.toLowerCase());
    });
    if (unmatched.length > 0) {
      issues.push({
        issue_type: 'logic',
        description: `Correct answer does not match provided options (${unmatched.join(', ')}).`,
        suggestion: 'Use option labels (A/B/C...) or exact option text that exists in this row.',
      });
    }
  }
  ...
  try {
    ...
    if (Array.isArray(choices) && choices.length > 0) {
      optionsBlock = choices
        .map((c: { identifier: string; text: string }, i: number) =>
    ...
    const correctAnswerText =
      Array.isArray(correctResponseIdentifiers) && correctResponseIdentifiers.length > 0
...
- Do not reject a question simply because it is difficult or tests advanced knowledge`;
    ...
    const model = (Deno.env.get('OPENROUTER_MODEL_ROW') ?? Deno.env.get('OPENROUTER_MODEL') ?? 'openrouter/auto').trim();
    ...
    if (result.status === 'ai_certified') {
      ...
      const nonEmptyIssues = result.issues.filter((iss) => String(iss.description ?? '').trim().length > 0);
      if (nonEmptyIssues.length === 0) {
        result.issues = inferStructuralIssues(questionType, stem, choices, correctResponseIdentifiers);
      }
      ...
      if (result.issues.length === 0) {
        result.issues = [
          {
            issue_type: 'clarity',
            description: 'AI marked this question as rejected but did not return a specific reason.',
            suggestion: 'Re-run AI audit. If it fails again, check stem, options, answer mapping, and delimiters.',
          },
        ];
      }
    }
      ...
      issues: [
        {
          ...
          suggestion: 'Please retry the audit. If the error persists, review the question manually.',
        },
      ],
      error: message,
    };
