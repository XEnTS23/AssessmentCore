...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { stem, questionType, choices, correctResponseIdentifiers, issues } = await req.json();
    ...
    const optionsBlock = Array.isArray(choices) && choices.length > 0
      ? choices.map((c: any, i: number) => `  ${alphabet[i] ?? i + 1}. ${c.text || c.identifier}`).join('\n')
      : '  (no options)';

    const correctText = Array.isArray(correctResponseIdentifiers) && correctResponseIdentifiers.length > 0
    ...
    const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert educational content editor. You fix assessment questions. Output only the corrected stem text — nothing else.',
          },
