...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { xmlContent, fileName, itemNo, provider, qtiVersion } =
      await req.json();

    if (!xmlContent || !provider || !qtiVersion) {
      return new Response(
        JSON.stringify({
          error: "Missing required fields: xmlContent, provider, qtiVersion",
        }),
        {
          status: 400,
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a strict QTI XML validator and educational content reviewer. Always respond with valid JSON only, no markdown formatting or code blocks.",
              },
    ...
    return new Response(
      JSON.stringify({
        ...
        issues: [
          {
            severity: "warning",
            message: `AI validation could not complete: ${message}`,
          },
        ],
        summary:
